#ifndef mulle_objc_time_foundation_include_h__
#define mulle_objc_time_foundation_include_h__

/* DO:    #include this files in public headers.

   DONT:  #include this files in private headers.
          #include this file directly in sources.
          #import this file anywhere (except in import.h)
 */


/* This is a central include file to keep dependencies out of the library
   C files. It is usually included by .h files only.

   As the name "include.h" is standardized across mulle-sde projects,
   .c and .h files are now  motile. They can be moved to other projects and
   don't need to be edited.

   If you delete it, it will not be regenerated.
*/

/*
 * Include the header file generated by mulle-sde reflect.
 */

#include "_MulleObjCTimeFoundation-include.h"

#if defined( MULLE_OBJC_TIME_FOUNDATION_BUILD) || defined( MULLE_FOUNDATION_BASE_BUILD)
# define MULLE_OBJC_TIME_FOUNDATION_GLOBAL        MULLE_C_GLOBAL
# define MULLE_OBJC_TIME_FOUNDATION_GLOBAL_VAR    MULLE_C_GLOBAL_VAR
#else
# if defined( MULLE_OBJC_TIME_FOUNDATION_INCLUDE_DYNAMIC) || (defined( MULLE_INCLUDE_DYNAMIC) && ! defined( MULLE_OBJC_TIME_FOUNDATION_INCLUDE_STATIC))
#  define MULLE_OBJC_TIME_FOUNDATION_GLOBAL   MULLE_C_GLOBAL
# else
#  define MULLE_OBJC_TIME_FOUNDATION_GLOBAL   extern
# endif
# define MULLE_OBJC_TIME_FOUNDATION_GLOBAL_VAR
#endif

/* You can add some more include statements here */

#endif


/*
 * extension : mulle-c/c
 * directory : project-oneshot/all
 * template  : .../include.h
 * Suppress this comment with `export MULLE_SDE_GENERATE_FILE_COMMENTS=NO`
 */
